...
module Serializers = 
    ...
    type ListSerializer<'T when 'T : equality>() = 
        ...
        override this.Deserialize(context : BsonDeserializationContext, args : BsonDeserializationArgs) : 'T list = 
            ...
            | _ -> 
                let msg = 
                    sprintf "Can't deserialize a %s from BsonType %s" args.NominalType.FullName (bsonType.ToString())
    ...
    let getClassMap isClassMapRegistered (actualType : Type) = 
        let rec getMember (_type : Type) name other = 
            ...
            if not (memberInfos |> Seq.isEmpty) then Some(Seq.head memberInfos)
            elif other <> null then getMember _type other null
            else None
        if not (isClassMapRegistered actualType) then 
            ...
            | Some SourceConstructFlags.RecordType -> 
                ...
                match actualType.GetConstructors()
                      |> Seq.sortBy (fun c -> c.GetParameters().Length)
                      |> Seq.tryHead with
                | Some c -> 
                    let parms = 
                        classMap.DeclaredMemberMaps
                        |> Seq.map (fun (m : BsonMemberMap) -> m.MemberName)
                        |> Array.ofSeq
    ...
    type RecordSerializer<'T>(classMap : BsonClassMap) = 
        ...
        let classMapSerializer = 
            Activator.CreateInstance((typedefof<MongoDB.Bson.Serialization.BsonClassMapSerializer<_>>
                                         .MakeGenericType(typedefof<'T>)), 
                                     [ classMap ]
                                     |> Seq.cast<Object>
                                     |> Seq.toArray) :?> IBsonSerializer
        ...
        interface IBsonDocumentSerializer with
            member x.TryGetMemberSerializationInfo(memberName : string, serializationInfo : byref<BsonSerializationInfo>) : bool = 
                let getMemberName (map : BsonMemberMap) = 
                    match map.MemberInfo.GetCustomAttribute(typedefof<BsonElementAttribute>) :?> BsonElementAttribute with
                    | null -> map.ElementName
                    | a -> a.ElementName
        ...
        interface IBsonIdProvider with
            ...
            member this.GetDocumentId(document : Object, id : Object byref, nominalType : Type byref, 
                                      idGenerator : IIdGenerator byref) = 
                match getter with
                | Some(i) -> 
    ...
    type OptionTypeSerializer<'T, 'U>() = 
        ...
        override x.Deserialize(ctx : BsonDeserializationContext, args : BsonDeserializationArgs) = 
            ...
            let typeName = reader.ReadString()                       
            let opt = 
                if typeName = "Some" then 
                    reader.ReadName("_v")
                    Some(Unchecked.unbox<'U> (BsonSerializer.Deserialize(reader, innerType)))
                else None
        ...
        override x.Serialize(ctx : BsonSerializationContext, args : BsonSerializationArgs, value : 'T) = 
            ...
            writer.WriteStartDocument()
            writer.WriteName("_t")
            if box (value) = null then writer.WriteString("None")
            else writer.WriteString("Some")
            if box (value) <> null then 
                writer.WriteName("_v")
                let v = value.GetType().GetProperty("Value").GetValue(value)
                let serializer = BsonSerializer.LookupSerializer(v.GetType())
                serializer.Serialize(ctx, v)
            writer.WriteEndDocument()
        ...
        interface IBsonSerializationProvider with
            member this.GetSerializer(typ : Type) = 
                ...
                | Some SourceConstructFlags.RecordType -> 
                    ...
                    | Some classMap -> 
                        Activator.CreateInstance((typedefof<RecordSerializer<_>>.MakeGenericType(typ)), classMap) :?> IBsonSerializer
                ...
                | Some SourceConstructFlags.SumType -> 
                    ...
                    if typ.IsGenericType && typ.GetGenericTypeDefinition() = typedefof<List<_>> then 
                        ...
                        if typ.Name = "FSharpOption`1" then 
                            let args = [|typ;typ.GetGenericArguments().[0]|] |> Array.ofSeq
                            typedefof<OptionTypeSerializer<_,_>>.MakeGenericType(args) |> Activator.CreateInstance :?> IBsonSerializer
                        else 
                            typedefof<UnionCaseSerializer<_>>.MakeGenericType(typ) |> Activator.CreateInstance :?> IBsonSerializer
                    else null
